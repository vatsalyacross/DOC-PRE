...
const Myprofile = () => {
  ...
  const updateUserProfileData = async () => {
    console.log("Updating user profile with data:", userData); // Debug log
    try{
  ...
  console.log("User Data in MyProfile:", userData); // Debug log
    ...
    <div className='max-w-lg flex flex-col gap-2 text-sm'>
      {
        ...
        ? (
          <label htmlFor="image">
            ...
            <input name="image" onChange={(e)=> setImage(e.target.files[0])} type="file" id="image" hidden />
          </label>
        )
      ...
      {
        isEdit
          ? <input className='bg-gray-50 text-3x1 font-medium max-w-60 mt-4' type="text" value={userData.name} onChange={e => setUserData(prev => ({ ...prev, name: e.target.value }))} />
          : <p className='font-medium text-3x1 text-neutral-800 mt-4'>{userData.name}</p>
      }
